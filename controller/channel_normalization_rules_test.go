...
func TestValidateChannelNormalizationSelectionRejectsChangedMappingTarget(t *testing.T) {
	candidate := channelNormalizationItem{
		ChannelID:  12,
		MappingSet: map[string]string{"claude-opus-4-7": "claude-opus-4.7"},
	}
	...
	_, err := validateChannelNormalizationSelection(candidate, applyChannelNormalizationSelection{
		ChannelID:  12,
		MappingSet: map[string]string{"claude-opus-4-7": "attacker-controlled-model"},
	})
	require.ErrorContains(t, err, "target changed after preview")
}
