...
func TestMidjourneyFailureRefundsWhenQuotaObservationIsUnavailable(t *testing.T) {
	...
	require.NoError(t, db.AutoMigrate(
		...
		&model.QuotaCycle{}, &model.QuotaPlan{}, &model.Log{}, &model.AuditLog{},
	))
	...
	require.Eventually(t, func() bool {
		...
		return db.Model(&model.AuditLog{}).Where("action = ?", model.AuditActionQuotaCycleSettle).Count(&auditCount).Error == nil && auditCount == 1
	}, time.Second, 10*time.Millisecond)
	var audit model.AuditLog
	require.NoError(t, db.Where("action = ?", model.AuditActionQuotaCycleSettle).First(&audit).Error)
	assert.True(t, audit.Success)
	require.NotNil(t, audit.Other.Op)
	var cycle model.QuotaCycle
	require.NoError(t, db.First(&cycle).Error)
	var auditedCycleID int
	auditedCycleJSON, err := common.Marshal(audit.Other.Op.Params["cycle_id"])
	require.NoError(t, err)
