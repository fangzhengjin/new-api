...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for {
		...
		for attempt := 0; attempt <= common.RetryTimes; attempt++ {
			...
			processChannelError(c, currentChannelError(c, channel), newAPIError, advanceTarget)
			retryIndex++
			if !canRetry {
				break
			}
		...
		retryParam.ExcludeTarget(channel.Id, isMultiKey, keyIndex)
		if isMultiKey {
			if setupErr := middleware.SetupContextForSelectedChannel(c, channel, relayInfo.OriginModelName, retryParam.ExcludedKeyIndexes(channel.Id)); setupErr == nil {
				continue
			}
	...
	if shouldRecordRelayFailure(c, newAPIError) {
		gopool.Go(func() {
...
func relayCanceled(c *gin.Context, err error) bool {
	return relayRequestCanceled(c) || errors.Is(err, context.Canceled)
}
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, allowDisable bool) {
	if relayCanceled(c, err) {
		return
	}
	...
	for {
		...
		for attempt := 0; attempt <= common.RetryTimes; attempt++ {
			...
			if !taskErr.LocalError {
				...
				processChannelError(c, currentChannelError(c, channel), apiErr, advanceTarget)
			}
			retryIndex++
			if !canRetry {
				break
...
func shouldRetryTaskRelay(c *gin.Context, taskErr *taskdto.TaskError) bool {
	...
	if relayCanceled(c, taskErr.Error) {
		return false
	}
	if service.ShouldSkipRetryAfterChannelAffinityFailure(c) {
		return false
