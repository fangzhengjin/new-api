...
func TestManageUserExecutesCycleQuotaAdjustmentWithReason(t *testing.T) {
	...
	require.NoError(t, db.Create(&cycle).Error)
	require.NoError(t, db.Create(&user).Error)
	// The quota executor resolves the acting operator by username, so the fixture must contain the account the request context claims.
	operator := model.User{Id: 9999, Username: "root-operator", Role: common.RoleRootUser, Status: common.UserStatusEnabled, AuthVersion: 1}
	require.NoError(t, db.Create(&operator).Error)
