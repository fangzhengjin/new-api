...
type successLimitReservation struct {
	...
	member string
	rdb    *redis.Client
	mem    *common.RateLimitReservation
}
...
func runModelRequestTrafficLimit(c *gin.Context, next func()) {
	...
	next()
	succeeded := modelRequestSucceeded(c) && (c.Writer.Written() || c.Request.Context().Err() == nil)
	keepReservations = succeeded
	if succeeded {
		for _, reservation := range reservations {
			reservation.completeMemory(succeeded)
		}
	}
}
...
// completeMemory settles an in-memory reservation: success records the request,
// failure frees the slot. Redis reservations keep their success entries because
// the release path only runs on failure.
func (r successLimitReservation) completeMemory(success bool) {
	if r.mem != nil {
		r.mem.Complete(success)
	}
}
...
func takeSuccessLimit(c *gin.Context, key string, maxCount int, duration int64, templateKey string, code types.ErrorCode) (successLimitReservation, bool) {
	...
	if common.RedisEnabled {
		...
		inMemoryRateLimiter.Init(time.Duration(setting.ModelRequestRateLimitDurationMinutes) * time.Minute)
		reservation.mem = inMemoryRateLimiter.Reserve(key, maxCount, duration)
		allowed = reservation.mem != nil
	}
	if allowed {
