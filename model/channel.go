...
func UpdateChannelStatus(channelId int, usingKey string, status int, reason string) bool {
	...
	beforeStatus := channel.Status
	// 手动渠道操作必须替换掉「全部 key 不可用」的自动禁用原因，即使状态值本身没有变化，
	// 否则自动恢复逻辑会把已被人工停用的渠道继续当作额度耗尽处理。该判定必须早于
	// handlerMultiKeyUpdate，因为它会就地改写 other_info 中的状态原因。
	...
	if !changed && !overridesKeyExhaustion {
		return false
	}
	if overridesKeyExhaustion {
		info := channel.GetOtherInfo()
		info["status_reason"] = reason
		info["status_time"] = common.GetTimestamp()
		channel.SetOtherInfo(info)
...
func updateChannelUsedQuota(id int, quota int) error {
	...
	if err != nil {
		return fmt.Errorf("failed to update channel used quota: channel_id=%d, delta_quota=%d, error=%w", id, quota, err)
	}
	return nil
}
