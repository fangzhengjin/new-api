...
func TestMigrateChatPresetsOptionPreservesStandardPreset(t *testing.T) {
	db := useFrontendOptionMigrationDB(t)
	standard := `[{"name":"无限画布","url":"https://new.example/image#code={authCode}","enabled":false,"icon":"Palette","open_mode":"new_tab"}]`
	require.NoError(t, db.Create(&Option{Key: "Chats", Value: standard}).Error)
...
func TestMigrateChatPresetsOptionDoesNotCreateMissingChats(t *testing.T) {
	for _, name := range []string{"fresh database", "retired options only"} {
		t.Run(name, func(t *testing.T) {
			db := useFrontendOptionMigrationDB(t)
			if name == "retired options only" {
				require.NoError(t, db.Create(&[]Option{
					{Key: "InfiniteCanvasEnabled", Value: "true"},
					{Key: "InfiniteCanvasLaunchURL", Value: "https://canvas.example/?code={code}"},
				}).Error)
			}
			var before []Option
