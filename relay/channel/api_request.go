...
var passthroughSkipHeaderNamesLower = map[string]struct{}{
	...
	// Host is carried by http.Request.Host rather than Header.
	"host": {},
	...
	// The upstream request builder replaces the client WebSocket subprotocol value.
...
func shouldSkipPassthroughHeader(name string) bool {
	...
	if !operation_setting.ResolveRequestHeaderPolicy(name, nil).Forward {
		return true
	}
...
func DoWssRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*websocket.Conn, error) {
	...
	removeNonForwardedRequestHeaders(targetHeader, operation_setting.GetRequestHeaderRules())
	billingRequest, err := http.NewRequestWithContext(c.Request.Context(), http.MethodGet, fullRequestURL, nil)
	if err != nil {
		return nil, fmt.Errorf("build billing request failed: %w", err)
	}
	billingRequest.Header = targetHeader
	if host := targetHeader.Get("Host"); host != "" {
		billingRequest.Host = host
	}
	if billingErr := service.PrepareTieredBillingForUpstreamRequest(c, info, billingRequest); billingErr != nil {
		return nil, billingErr
	}
	service.CaptureUpstreamRequestHeadersAudit(c, targetHeader)
