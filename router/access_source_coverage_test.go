...
func TestAccessSourceLimitCoversConsumptionRoutesOnly(t *testing.T) {
	...
	consumptionRoutes := []struct {
		...
		{name: "playground", method: http.MethodPost, path: "/pg/chat/completions", credential: "coverage-dashboard-pat"},
		{name: "midjourney submit", method: http.MethodPost, path: "/mj/submit/imagine", credential: "coveragekey"},
		{name: "generic task submit", method: http.MethodPost, path: "/v1/tasks/demo", credential: "coveragekey"},
		{name: "legacy video submit", method: http.MethodPost, path: "/v1/video/generations", credential: "coveragekey"},
		{name: "video remix", method: http.MethodPost, path: "/v1/videos/task-1/remix", credential: "coveragekey"},
		{name: "video protocol submit", method: http.MethodPost, path: "/v1/videos", credential: "coveragekey"},
		// WebSocket 握手表面上不产生 token，但同样按消费请求计入访问来源。
		{name: "responses websocket", method: http.MethodGet, path: "/v1/responses", credential: "coveragekey"},
		{name: "realtime websocket", method: http.MethodGet, path: "/v1/realtime", credential: "coveragekey"},
	}
