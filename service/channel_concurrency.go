...
const (
	channelConcurrencyLease = time.Minute
	channelConcurrencyPoll  = 200 * time.Millisecond
)
...
func AcquireChannelConcurrency(ctx context.Context, channelID, limit int, waitTimeout time.Duration) (context.Context, func(), error) {
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				if limit == 0 {
					_, _ = common.RDB.Eval(leaseCtx, acquireChannelConcurrencyScript, []string{key, waitingKey}, 0, member, channelConcurrencyLease.Milliseconds()).Result()
					continue
				}
