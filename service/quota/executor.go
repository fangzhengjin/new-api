...
func executePlanInTransaction(tx *gorm.DB, planID int, cycleID int, executedBy string, executedAt int64) (*executionCommit, error) {
	...
	actor := model.AuditAdminInfo{AdminUsername: executedBy}
	if executedBy == "system" && (plan.PlanType == model.QuotaPlanTypeSettlement || temporaryGrantOnly) {
		actor.AuthMethod = "system"
	} else {
		var operator model.User
		if err := tx.Select("id", "username", "role").Where("username = ?", executedBy).First(&operator).Error; err != nil {
			return nil, fmt.Errorf("读取额度执行操作者失败: %w", err)
		}
		if operator.Role != common.RoleAdminUser && operator.Role != common.RoleRootUser {
			return nil, errors.New("额度执行操作者必须为管理员")
		}
		actor.AdminID, actor.AdminRole = operator.Id, operator.Role
	}
	var storedParameters map[string]json.RawMessage
	if err := common.UnmarshalJsonStr(plan.Parameters, &storedParameters); err != nil {
		return nil, err
	}
	if storedParameters == nil {
		storedParameters = make(map[string]json.RawMessage)
	}
	encodedActor, err := common.Marshal(actor)
	if err != nil {
		return nil, err
	}
	storedParameters["execution_actor"] = encodedActor
	parameters, err := common.Marshal(storedParameters)
	if err != nil {
		return nil, err
	}
	...
	for index := range items {
		...
		if sameLogDatabase {
			...
			item.ActualBeforeQuota, item.ActualAfterQuota = &before, &after
			if err := model.CreateAuditLog(tx, quotaAuditLog(plan, *item, actor, executedAt)); err != nil {
				return nil, err
			}
...
// quotaAuditLog keeps the recipient as owner and the execution actor separate.
func quotaAuditLog(plan model.QuotaPlan, item model.QuotaItem, actor model.AuditAdminInfo, executedAt int64) model.AuditLog {
	requestID := quotaLogRequestID(plan.Id, item.Id)
	return model.AuditLog{
		EventId: requestID, RequestId: requestID, UserId: item.UserId, Username: item.Username,
		ActorRole: actor.AdminRole, CreatedAt: executedAt, Category: model.AuditCategoryOperation,
		Action: model.AuditActionQuotaBalanceAdjust, Success: true, Content: item.LogContent,
		AuthMethod: actor.AuthMethod,
		Other: model.AuditOther{AdminInfo: &actor, Op: &model.AuditOperation{
			Action: model.AuditActionQuotaBalanceAdjust,
			Params: model.AuditFields{"plan_id": plan.Id, "cycle_id": plan.CycleId, "item_id": item.Id,
				"id": item.UserId, "username": item.Username, "adjustment_action": item.Action,
				"before_quota": item.ActualBeforeQuota, "after_quota": item.ActualAfterQuota, "adjustment_quota": item.AdjustmentQuota},
		}},
	}
}
