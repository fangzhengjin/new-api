...
func settleCycle(cycleID int, now int64, updatedBy string) error {
	...
	if updatedBy == "system" {
		model.RecordAuditLog(nil, model.AuditLog{
			UserId: 0, ActorRole: 0, Username: "system", AuthMethod: "system",
			Category: model.AuditCategoryOperation, Action: model.AuditActionQuotaCycleSettle,
			Content: fmt.Sprintf("Settled quota cycle %d", cycleID), Success: true,
			Other: model.AuditOther{Op: &model.AuditOperation{
				Action: model.AuditActionQuotaCycleSettle,
				Params: model.AuditFields{"cycle_id": cycleID, "balance_policy": balancePolicy},
			}},
		})
	}
	return nil
