...
func TestCreateCycleRejectsReplayWhenCycleQuotaManagementIsDisabled(t *testing.T) {
	...
	require.NoError(t, db.Model(&model.Option{}).Where(map[string]interface{}{"key": model.CycleQuotaManagementOptionKey}).Update("value", "false").Error)
	now := time.Now().Unix()
...
func TestManualIncreaseUsesCycleAllocationWithoutPlanConfirmation(t *testing.T) {
	...
	for _, role := range []int{common.RoleAdminUser, common.RoleRootUser} {
		...
		_, err := ManualAdjustUserQuota(user.Id, common.RoleAdminUser, mustQuota(t, "150"), "业务需要", "admin")
		require.ErrorIs(t, err, model.ErrUserQuotaPermission)
		require.NoError(t, db.First(&user, user.Id).Error)
		assert.Equal(t, int(mustQuota(t, "50")), user.Quota)
		var count int64
	...
	result, err := ManualAdjustUserQuota(user.Id, common.RoleRootUser, mustQuota(t, "150"), "业务需要", "root")
	require.NoError(t, err)
...
func TestManualIncreaseCannotExceedCycleBudget(t *testing.T) {
	...
	_, err := ManualAdjustUserQuota(user.Id, common.RoleRootUser, mustQuota(t, "20"), "业务需要", "root")
...
func TestManualIncreaseMayLeaveBalanceNegative(t *testing.T) {
	...
	result, err := ManualAdjustUserQuota(user.Id, common.RoleRootUser, 30, "signed balance", "root")
	require.NoError(t, err)
	assert.Equal(t, 1, result.AffectedUsers)
	require.NoError(t, db.First(&user, user.Id).Error)
	...
	_, err = ManualAdjustUserQuota(user.Id, common.RoleRootUser, -1, "signed balance", "root")
...
func TestManualIncreaseUsesFinalBalanceRange(t *testing.T) {
	...
	result, err := ManualAdjustUserQuota(user.Id, common.RoleRootUser, int64(common.MaxQuota)+1, "return to zero", "root")
	require.NoError(t, err)
	assert.Equal(t, 1, result.AffectedUsers)
	require.NoError(t, db.First(&user, user.Id).Error)
...
func TestManualDecreaseUsesAdjustmentAmount(t *testing.T) {
	...
	result, err := ManualAdjustUserQuota(user.Id, common.RoleRootUser, -mustQuota(t, "30"), "manual decrease", "root")
	require.NoError(t, err)
	assert.Equal(t, 1, result.AffectedUsers)
...
func TestManualAdjustmentRejectsIneligibleDecreaseTargets(t *testing.T) {
	...
	_, err := ManualAdjustUserQuota(users[0].Id, common.RoleRootUser, -mustQuota(t, "20"), "too much", "root")
	require.EqualError(t, err, "调减额度不能超过用户当前余额")
	_, err = ManualAdjustUserQuota(users[1].Id, common.RoleRootUser, -mustQuota(t, "20"), "whitelist", "root")
...
func TestManualAdjustmentRequiresReasonAndCountsCharacters(t *testing.T) {
	...
	_, err := ManualAdjustUserQuota(user.Id, common.RoleRootUser, 1, "", "root")
	require.EqualError(t, err, "调整原因不能为空")
	_, err = ManualAdjustUserQuota(user.Id, common.RoleRootUser, 1, strings.Repeat("调", 255), "root")
	require.NoError(t, err)
	_, err = ManualAdjustUserQuota(user.Id, common.RoleRootUser, 1, strings.Repeat("调", 256), "root")
...
func TestManualAdjustmentRollsBackDraftWhenExecutionFails(t *testing.T) {
	...
	_, err := ManualAdjustUserQuota(user.Id, common.RoleRootUser, 1, "rollback execution", "root")
...
func TestSetQuotaWhitelistChangesImmediatelyWithoutCurrentCycle(t *testing.T) {
	...
	// The service must use the current locked role, not a controller's earlier read.
	require.NoError(t, db.Model(&user).Update("role", common.RoleAdminUser).Error)
	require.ErrorIs(t, SetQuotaWhitelist(user.Id, common.RoleAdminUser, true, "admin"), model.ErrUserQuotaPermission)
	require.NoError(t, db.First(&user, user.Id).Error)
	assert.False(t, user.QuotaWhitelist)
	require.NoError(t, SetQuotaWhitelist(user.Id, common.RoleRootUser, true, "root"))
	require.NoError(t, SetQuotaWhitelist(user.Id, common.RoleRootUser, true, "root"))
	require.NoError(t, db.First(&user, user.Id).Error)
	assert.True(t, user.QuotaWhitelist)
	assert.Equal(t, int(mustQuota(t, "25")), user.Quota)

	require.NoError(t, SetQuotaWhitelist(user.Id, common.RoleRootUser, false, "root"))
	require.NoError(t, SetQuotaWhitelist(user.Id, common.RoleRootUser, false, "root"))
	require.NoError(t, db.First(&user, user.Id).Error)
	assert.False(t, user.QuotaWhitelist)
	assert.Equal(t, int(mustQuota(t, "25")), user.Quota)
...
func TestSetQuotaWhitelistRejectsReplayOutsideCycleQuotaManagement(t *testing.T) {
	...
	require.EqualError(t, SetQuotaWhitelist(user.Id, common.RoleRootUser, true, "root"), "周期额度管理未启用，不能修改额度白名单")
	require.NoError(t, db.First(&user, user.Id).Error)
	assert.False(t, user.QuotaWhitelist)
}
