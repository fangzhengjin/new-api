...
func deliverQuotaLog(planID int, itemID int) bool {
	...
	transactionErr := model.DB.Transaction(func(tx *gorm.DB) error {
		...
		// Select scalar fields so ClickHouse JSON decoding is not needed for deduplication.
		if err := logDB.Select("event_id", "request_id", "user_id", "username", "category", "action", "content").
			Where("event_id = ? OR request_id = ?", requestID, requestID).Limit(2).Find(&audits).Error; err != nil {
			return markLogFailure(tx, item.Id, err)
		}
		if len(audits) > 1 {
			return markLogFailure(tx, item.Id, errors.New("审计事件标识对应多条记录"))
		}
		if len(audits) == 1 {
			entry := audits[0]
			if entry.EventId != requestID || entry.RequestId != requestID || entry.UserId != item.UserId ||
				entry.Username != item.Username || entry.Category != model.AuditCategoryOperation ||
				entry.Action != model.AuditActionQuotaBalanceAdjust || entry.Content != item.LogContent {
				return markLogFailure(tx, item.Id, errors.New("审计事件标识已被其他记录占用"))
			}
		}
		if len(audits) == 0 {
			...
			if plan.Status != model.QuotaPlanStatusExecuted || plan.ExecutedAt == nil {
				return markLogFailure(tx, item.Id, errors.New("方案尚未执行"))
			}
			var parameters PlanParameters
			if err := common.UnmarshalJsonStr(plan.Parameters, &parameters); err != nil {
				return markLogFailure(tx, item.Id, err)
			}
			actor := parameters.ExecutionActor
			if actor == nil {
				// Old executions did not snapshot roles; never infer historical
				// privileges from an account's present-day role or replacement account.
				actor = &model.AuditAdminInfo{AdminUsername: plan.ExecutedBy, AuthMethod: "legacy"}
			}
			if err := model.CreateAuditLog(logDB, quotaAuditLog(plan, item, *actor, *plan.ExecutedAt)); err != nil {
				return markLogFailure(tx, item.Id, err)
			}
		}
		now := time.Now().Unix()
...
func deliverQuotaEmail(planID int, itemID int) bool {
	...
	transactionErr := model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := common.SendEmail(subject, item.Email, htmlContent); err != nil {
			return markEmailFailure(tx, item.Id, err)
		}
		now := time.Now().Unix()
		if err := tx.Model(&model.QuotaItem{}).Where("id = ?", item.Id).Updates(map[string]interface{}{
...
func quotaEmailContent(item model.QuotaItem) (string, string, error) {
	subjects := map[model.QuotaAdjustmentAction]string{
		...
		model.QuotaAdjustmentActionTemporaryGrant: "AI临时额度发放通知",
	}
	accents := map[model.QuotaAdjustmentAction]string{
		...
		model.QuotaAdjustmentActionTemporaryGrant: "#7c3aed",
	}
	subject, ok := subjects[item.Action]
	if !ok {
		return "", "", fmt.Errorf("不支持发送额度动作 %s 的邮件", item.Action)
	}
...
<div style="padding:16px;background:#fafafa;border-left:4px solid %s;line-height:1.6;white-space:pre-wrap;overflow-wrap:anywhere">%s</div>
	...
	return subject, htmlContent, nil
}
