...
func SetQuotaWhitelist(userID, operatorRole int, enabled bool, operator string) error {
	if userID <= 0 {
	...
	err := model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if operatorRole != common.RoleRootUser && operatorRole <= user.Role {
			return model.ErrUserQuotaPermission
		}
...
// and records it as an immutable executed plan item after checking operatorRole against the locked target.
func ManualAdjustUserQuota(userID, operatorRole int, adjustment int64, reason, operator string) (*ExecuteResult, error) {
	if !model.CycleQuotaManagementEnabled() {
