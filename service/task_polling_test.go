...
func TestUpdateBatchTasksRefundsWhenQuotaObservationIsUnavailable(t *testing.T) {
	...
	// 结算审计由 gopool 后台写入，等待审计行本身出现；不能等全局协程池排空，
	// 池中可能还挂着其他测试触发的异步渠道禁用任务。
	require.Eventually(t, func() bool {
		var settled int64
		if err := model.LOG_DB.Model(&model.AuditLog{}).Where("action = ?", model.AuditActionQuotaCycleSettle).Count(&settled).Error; err != nil {
			return false
		}
		return settled == 1
