...
// The re-frozen reservation must keep the pre-consume multiplier for
// token-priced leaves so a group retry raises the reservation to the same
// multiplied basis the initial pre-consume used (expr.md reservation section).
func TestPrepareTieredBillingForUpstreamRequestKeepsMultiplier(t *testing.T) {
	const expr = `tier("base", p) * (param("service_tier") == "fast" ? 2 : 1)`
	billing := &recordingBillingSettler{preConsumedQuota: 500_000}
	relayInfo := &relaycommon.RelayInfo{
		Billing:               billing,
		...
		TieredBillingSnapshot: &billingexpr.BillingSnapshot{
			...
			ExprHash:                  billingexpr.ExprHashString(expr),
			GroupRatio:                1,
			PreConsumeMultiplier:      1.5,
			EstimatedPromptTokens:     1_000_000,
			EstimatedQuotaBeforeGroup: 750_000,
			EstimatedQuotaAfterGroup:  750_000,
			EstimatedTier:             "base",
			QuotaPerUnit:              500_000,
		},
		PriceData: types.PriceData{GroupRatioInfo: types.GroupRatioInfo{GroupRatio: 1}},
	...
	require.Nil(t, PrepareTieredBillingForUpstreamRequest(nil, relayInfo, req))
	// tier("base", 1M) * 2 evaluates to 2M tokens → 1M quota before the group
	// ratio; with a 1.5 multiplier the raised reservation must be 1.5M, not 1M.
	assert.Equal(t, 1_500_000, relayInfo.TieredBillingSnapshot.EstimatedQuotaAfterGroup)
	assert.Equal(t, []int{1_500_000}, billing.reserveTargets)
}
