...
function SourceHarness(props: { onReset?: () => void }) {
  const form = useForm<DiscoveryFormValues>({
    defaultValues: {
...
describe('channel discovery workspace pending state', () => {
  test('closes reset confirmation after keyboard confirmation and preserves drafts on cancel', async () => {
    const user = userEvent.setup()
    const onReset = vi.fn()
    render(<SourceHarness onReset={onReset} />)

    screen.getByRole('button', { name: 'Reset' }).focus()
    await user.keyboard('{Enter}')
    const dialog = await screen.findByRole('alertdialog', { name: 'Reset' })
    ...
    const reopened = await screen.findByRole('alertdialog', { name: 'Reset' })
    await waitFor(() =>
      expect(
    ...
    await user.tab()
    expect(
      within(reopened).getByRole('button', { name: 'Reset' })
    ).toHaveFocus()
    await user.keyboard('{Enter}')
  ...
  test('identifies the model and key while a protocol probe is running', async () => {
    ...
    assert.ok(status)
    assert.match(status.textContent ?? '', /Probing upstream-model with key 2/)
  ...
  test('renders long group names without dropping the option', async () => {
    const container = document.createElement('div')
    document.body.append(container)
    const root = createRoot(container)
    ...
    const longName = container.querySelector(
      'label[for="discovery-group-c:events:free:claude:very-long-name"] span'
    )
    assert.ok(group)
    assert.ok(longName)
