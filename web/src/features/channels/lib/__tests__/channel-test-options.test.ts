...
import {
  ...
  resolveChannelTestStream,
} from '../channel-actions'

describe('channel test stream options', () => {
  test('defaults to streaming and preserves an explicit synchronous test', () => {
    assert.equal(resolveChannelTestStream(), true)
    assert.equal(resolveChannelTestStream(false), false)
  })
