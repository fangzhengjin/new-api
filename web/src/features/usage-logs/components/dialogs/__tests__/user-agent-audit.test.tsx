...
describe('Request header audit details', () => {
  test('shows the narrow User-Agent transition and expands other headers only to administrators', async () => {
    ...
    expect(within(metadataGroup).getByText(codexTurnMetadata)).toHaveClass(
      'line-clamp-2'
    )
    ...
    expect(within(metadataGroup).getByText(codexTurnMetadata)).not.toHaveClass(
      'line-clamp-2'
    )
  ...
  test('keeps the header scroller vertically stable while pressing details', async () => {
    ...
    const group = screen.getByRole('group', {
      name: `User-Agent: ${wideUserAgent}`,
    })
    expect(details).toHaveClass('active:not-aria-[haspopup]:translate-y-0')
    expect(details).not.toHaveClass('active:not-aria-[haspopup]:translate-y-px')
    expect(group.parentElement).toHaveClass('overflow-y-hidden')
  })
})
