    ...
    <DetailSection label={t('Token Breakdown')}>
      <div className='-m-2.5 max-sm:-m-2'>
        ...
        <div className='space-y-2 px-3 py-2'>
          ...
          {other.image_cache_tokens !== undefined ? (
            <dl className='flex items-baseline justify-between gap-4'>
              <dt className='text-muted-foreground text-xs'>
                {t('Image Cache')}
              </dt>
              <dd className='text-xs tabular-nums'>
                {other.image_cache_tokens.toLocaleString()}
              </dd>
            </dl>
          ) : null}
...
export function DetailsDialog(props: DetailsDialogProps) {
  ...
  return (
    <Dialog
      ...
      contentClassName='min-w-0 overflow-hidden max-sm:max-h-(--dialog-available-height) max-sm:w-[calc(100vw-1.5rem)] max-sm:max-w-[calc(100vw-1.5rem)] max-sm:p-4 sm:max-w-4xl lg:max-w-5xl'
